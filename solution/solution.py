...
def call_openai(
    ...
    # perf_counter: đồng hồ đơn điệu, độ phân giải cao — time.time() trên
    # Windows có bước nhảy ~15ms nên với lời gọi nhanh sẽ ra đúng 0.0.
...
def compare_models(prompt: str) -> dict:
    ...
    # Ước lượng thô: ~0.75 từ cho 1 token. Chỉ tính phần output để so sánh
    # nhanh giữa hai model; Part 2 sẽ đếm token thật bằng tiktoken.
...
def chat_with_system_prompt(
    ...
    # Thứ tự có ý nghĩa: system đứng đầu để định hình vai trò trước khi model
    # đọc câu hỏi thật của người dùng.
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": user_prompt},
    ]
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # encoding_for_model ném KeyError với model lạ (ví dụ model NIM),
        # và lần chạy đầu cần tải bộ mã hóa về nên có thể lỗi mạng.
        enc = tiktoken.encoding_for_model(model)
        return len(enc.encode(text))
    except Exception:
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    # Model ngoài bảng giá (ví dụ model NIM miễn phí) lấy giá gpt-4o làm
    # tham chiếu học tập, thay vì để KeyError làm hỏng cả phiên chat.
    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])
    ...
    return {
        ...
        # Không làm tròn: test so khớp tổng tới 10 chữ số thập phân.
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        reply = ""
        for chunk in stream:
            # Chunk cuối của stream thật có delta.content = None,
            # `or ""` tránh TypeError khi nối chuỗi.
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
            reply += delta
        print()
        ...
        # Giữ 3 lượt gần nhất = 6 message, để input token không phình theo phiên.
        history = history[-6:]
...
def retry_with_backoff(
    ...
    # Tổng số lần gọi = 1 lần đầu + max_retries lần thử lại.
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                break
            # Backoff cấp số nhân: 0.1s → 0.2s → 0.4s ...
            time.sleep(base_delay * (2 ** attempt))

    # Ném lại đúng exception gốc để lớp gọi biết nguyên nhân thật.
    raise last_error
...
def run_assistant(
    ...
    while True:
        ...
        # mà không chờ người dùng gõ phím.
        if max_turns is not None and num_turns >= max_turns:
            break
        ...
        # Persona luôn đứng đầu mỗi request nên không bị trôi mất khi
        # history bị cắt bớt.
        messages = (
        ...
        # lambda để retry_with_backoff gọi lại được toàn bộ request khi lỗi.
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def clip(text: str, width: int = 40) -> str:
        """Cắt text dài và thay xuống dòng bằng khoảng trắng cho gọn bảng."""
        flat = " ".join(str(text).split())
        return flat if len(flat) <= width else flat[: width - 1] + "…"

    headers = ("Prompt", "GPT-4o Response", "Mini Response", "4o Lat.", "Mini Lat.")
    widths = (30, 40, 40, 9, 9)

    rows = [
        (
            clip(r["prompt"], 30),
            clip(r["gpt4o_response"]),
            clip(r["mini_response"]),
